...
c-----
c            write(LOT,*) ' '
c            write(LOT,*) 'Total period=',nper
c            write(LOT,*) ' '
c            write(LOT,*)
c     *      'Number of modes at the lowest period:',t0,' = ',kmode
c-----
...
c-----
        ...
        real*4 dcdh(NL), dcda(NL), dcdb(NL), dcdr(NL)
        real*4 ur(NL), tur(NL), uz(NL), tuz(NL), xlam(NL)

        character mname*80, cfile*80
        integer ipar(10)
        real*4 fpar(10)
...
c-----
   ...
   31    format(//22x,'RAYLEIGH WAVE      MODE #',i3/
     ...
     4  ,'         TZ        DC/DH'
     5  ,'      DC/DA      DC/DB      DC/DR'
     6  ,'      Lambda      Wave number')
c   41    format(//3x,'Frequency     Admittance'
c     1  ,'         UR           TR           UZ'
c     2  ,'           TZ           PhaVel'
c     3  ,'           Lambda           Wave Number')
c-----
...
c-----
     ...
     1          mname,ipar,fpar)
            ...
            l=lgstr(mname)
            cfile='compliance_'//mname(1:l-4)//'.txt'
            open(4,file=cfile,status='unknown',
     1          access='sequential',form='formatted')
            rewind 4
...
c------
  ...
  200   continue
    ...
    1   format(i5,1x,5e11.3)
     ...
     1      dcda(i),dcdb(i),dcdr(i),xlam(i),wvno,i=1,nmax)
        do i=1,nmax
            if(tur(i)==0.0)then
                if(tur(i+1)/=0.0)then
                    ad = uz(i)/(-tuz(i))
c     1      +wvno*xlam(i-1)*ur(i-1))
                    write(4,4) 1/t0,ad,ur(i),tur(i),
     1      uz(i),tuz(i),c,xlam(i),wvno
    4               format(f6.4,8(1x,e11.5))
                endif
            endif
        enddo
    2   format(i5,1x,10e11.3)
