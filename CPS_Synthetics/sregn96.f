...
c-----
     ...
     *                 dcda(NL),dcdb(NL),dcdr(NL),dcdh(NL)
        ...
        real*4 spur(NL), sptr(NL), spuz(NL), sptz(NL), sxlam(NL)
...
c            write(*,*) rho(i), a(i), b(i)
c            slam(i)=rho(i)*(a(i)*a(i)-2.*b(i)*b(i))
c            write(*,*) slam(i)
            depup = deplw + d(i)
...
c                zb(i) = b(i)
c                zrho(i) = rho(i)
c                xlam(i)=dble(rho(i)*(a(i)*a(i)-2.*b(i)*b(i)))
c  507       continue
     ...
     5          spur,sptr,spuz,sptz,ipar,sxlam)
            if(verbose)then
              write(6,*)sur,suz,sur0
              write(6,*)rur,ruz,rur0
              do i=1,mmaxot
                  write(6,*)i,spur(i),spuz(i)
              enddo
